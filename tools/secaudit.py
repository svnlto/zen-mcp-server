...
SECAUDIT_WORKFLOW_FIELD_DESCRIPTIONS = {
    "step": (
        "Describe current security audit investigation. Step 1: State audit plan, pass files via relevant_files. "
        "Follow OWASP Top 10, examine auth/authz, input validation, dependencies. Consider security implications, threat vectors, attack surface, "
        "threat landscape. Later steps: trace security dependencies, verify assumptions."
    ),
    "step_number": "Current step index, starting at 1. Each step builds on previous.",
    "total_steps": "Estimated steps to complete security audit. Adjust as findings emerge.",
    "next_step_required": "True to continue investigation, False when security audit complete and all threats uncovered.",
    "findings": (
        "Summarize security discoveries: vulnerabilities, auth issues, validation gaps, encryption, config, compliance. "
        "Document positive practices and concerns. Be specific, update in later steps."
    ),
    "files_checked": "All files examined during security audit. Use full absolute paths, do not shorten.",
    "relevant_files": (
        "Step 1: code files to audit. Final step: files with security issues or important patterns. Use full absolute paths, do not shorten."
    ),
    "relevant_context": (
    ...
    "issues_found": (
        "Security issues: dict with 'severity' (critical/high/medium/low) and 'description'. Include vulnerabilities, auth bypasses, injection, crypto, config issues."
    ),
    "confidence": (
        "Security audit confidence: 'exploring' (starting), 'low' (early), 'medium' (some evidence), "
        "'high' (strong evidence), 'very_high', 'almost_certain', 'certain' (complete audit)."
    ),
    "backtrack_from_step": (
    ...
    "security_scope": (
        "Security scope and application context (web app, API, etc). Include tech stack, users, data sensitivity, threats."
    ),
    "threat_level": (
        "Assess the threat level: 'low' (internal tools), 'medium' (customer-facing), 'high' (regulated), 'critical' (payment/PII)."
    ),
    "compliance_requirements": "Applicable compliance frameworks (SOC2, PCI DSS, HIPAA, GDPR, ISO 27001, NIST).",
